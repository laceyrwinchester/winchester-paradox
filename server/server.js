...
app.post('/api/subscribe', (req, res) => {
  ...
  const data = readData();
  if (data.subscribers.find(s => s.email === email)) return res.status(409).json({ error: 'Already subscribed' });
...
app.delete('/api/subscribers/:id', (req, res) => {
  const data = readData();
  data.subscribers = data.subscribers.filter(s => s.id !== req.params.id);
  writeData(data);
...
app.post('/api/login', (req, res) => {
  ...
  if (username === data.admin.username && password === data.admin.password) {
    res.json({ success: true, token: 'paradox_token_' + Date.now() });
  } else {
...
app.listen(PORT, () => console.log(`🌌 Winchester Paradox Server running on port ${PORT}`));
